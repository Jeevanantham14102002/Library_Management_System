...
def delete_user():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('DELETE  * FROM user WHERE userids', (deleteUserId, ))
        mysql.connection.commit()   
        return redirect(url_for('users'))
    return redirect(url_for('login'))
...
def books():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT book.bookid, book.picture, book.name, book.status, book.isbn, book.no_of_copy, book.updated_on, author.name as author_name, publisher.name AS publisher_name FROM book LEFT JOIN author ON author.authorid = book.authorid  LEFT JOIN publisher ON publisher.publisherid = book.publisherid")
...
def edit_book():
    ...
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT book.bookid, book.picture, book.name, book.status, book.isbn, book.no_of_copy, book.updated_on, book.authorid, book.publisherid, author.name as author_name, publisher.name AS publisher_name FROM book LEFT JOIN author ON author.authorid = book.authorid LEFT JOIN publisher ON publisher.publisherid = book.publisherid WHERE book.bookid = % s', (editBookId, ))
        books = cursor.fetchall()     
...
def save_book():
    ...
    if 'loggedin' in session:
        ...
        cursor.execute("SELECT book.bookid, book.picture, book.name, book.status, book.isbn, book.no_of_copy, book.updated_on, author.name as author_name, publisher.name AS publisher_name FROM book LEFT JOIN author ON author.authorid = book.authorid LEFT JOIN publisher ON publisher.publisherid = book.publisherid")
        books = cursor.fetchall() 
        if request.method == 'POST' and 'name' in request.form and 'author' in request.form and 'publisher' in request.form :
            ...
            if action == 'updateBook':
                ...
                cursor.execute('UPDATE book SET name= %s, status= %s, isbn= %s, no_of_copy= %s, authorid=%s,  publisherid= %s WHERE bookid = %s', (bookName, status, isbn, no_of_copy, author, publisher, (bookId, ), ))
                mysql.connection.commit()   
            else:
                cursor.execute('INSERT INTO book (`name`, `status`, `isbn`, `no_of_copy`, `authorid`,  `publisherid`) VALUES (%s, %s, %s, %s, %s, %s)', (bookName, status, isbn, no_of_copy
    , author, publisher))
                mysql.connection.commit()           
            return redirect(url_for('books'))        
        elif request.method == 'POST':
